...
const DmsPage = () => {
  ...
  const conversations = useQuery(api.directMessages.listConversations, { workspaceId });
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {(conversations ?? []).map((item) => {
              const displayName = formatDisplayName(item.otherUser?.name);
              const initials = getInitials(displayName);
              const preview = String(item.lastMessage?.preview ?? "");
              ...
              return (
                <Button
                  ...
                  onClick={() => {
                    router.push(
                      `/workspaces/${workspaceId}/members/${String(item.otherMemberId)}`
                    );
                  }}
                >
                  <Avatar className="size-11">
                    <AvatarImage alt={displayName} src={item.otherUser?.image ?? undefined} />
