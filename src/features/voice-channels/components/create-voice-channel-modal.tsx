...
import { useCreateVoiceChannelModal } from "../store/use-create-voice-channel-modal";
import { useCreateVoiceChannel } from "../api/use-create-voice-channel";

export const CreateVoiceChannelModal = () => {
  const router = useRouter();
  const workspaceId = useWorkspaceId();
  const [open, setOpen] = useCreateVoiceChannelModal();

  const { mutate, error, isPending, isSuccess, isError } = useCreateVoiceChannel();
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    await mutate(
      { workspaceId, name },
      {
        onSuccess: ({ voiceChannelId }) => {
          toast.success("Voice channel created successfully!");
          form.reset();
          handleClose();
          router.replace(`/workspaces/${workspaceId}/voice-channels/${voiceChannelId}`);
        },
      }
    );
  };
        ...
        <DialogContent className="fixed top-1/2 left-1/2 max-h-[85vh] w-[90vw] max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-6 shadow-lg focus:outline-none data-[state=open]:animate-slide-in-from-bottom-96 text-black">
          <DialogHeader>
            ...
            <DialogDescription className="text-purple-500">
              Please enter the name for your new voice channel.
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {isError ? (
              <p className="text-sm text-red-600" role="alert">
                {error?.message ?? "Failed to create voice channel. Please try again."}
              </p>
            ) : null}
            ...
            <div className="flex justify-end gap-2">
              <DialogFooter>
                ...
                >
                  {isPending ? "Creating..." : "Create Voice Channel"}
                </Button>
              </DialogFooter>
            </div>
          </form>
